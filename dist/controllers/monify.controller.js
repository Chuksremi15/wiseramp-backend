...
export class MonifyController extends BaseController {
    ...
    createReserveAccount = this.asyncHandler(async (req, res) => {
        try {
    ...
    getReserveAccount = this.asyncHandler(async (req, res) => {
        try {
            const { email } = req.body;
            if (!email) {
                return res
            ...
            if (error.message === "User does not have a reserve account reference") {
                return res.status(400).json({
                    success: false,
                    message: error.message,
                });
    ...
    confirmTransfer = this.asyncHandler(async (req, res) => {
        try {
            ...
            const response = await monifyAxios.post("/api/v2/disbursements/single/validate-otp", confirmationData);
            return res.status(200).json({
                success: true,
                data: response.data,
            });
    ...
    handlePaymentWebhook = this.asyncHandler(async (req, res) => {
        try {
            ...
            if (!eventType || !eventData) {
                console.error("Invalid webhook structure: missing eventType or eventData");
    ...
    async handleSuccessfulPayment(eventData) {
        try {
            ...
            if (user) {
                ...
                const pendingTransactions = await this.transactionService.getPendingTransactionsByChainAndEmail("fiat", user.email);
                let matchedTransaction = null;
                console.log(`[Fiat] Looking for amount: ${amountPaid} (type: ${typeof amountPaid})`);
                console.log(`[Fiat] Found ${pendingTransactions.length} pending transactions`);
                for (const transaction of pendingTransactions) {
                    const transactionAmount = parseFloat(transaction.sourceAmount);
                    const paidAmount = parseFloat(amountPaid);
                    console.log(`[Fiat] Comparing transaction ${transaction.transactionId}: ${transactionAmount} vs ${paidAmount}`);
                    if (Math.abs(transactionAmount - paidAmount) < 0.01) {
                        matchedTransaction = transaction;
                        console.log(`[Fiat] Matched transaction ${transaction.transactionId} with amount ${transaction.sourceAmount} NGN`);
                        break;
                    }
                }
                console.log(`[Fiat] matchedTransaction after loop:`, matchedTransaction
                ...
                console.log(`[Fiat] 🔍 Checking transaction status: ${matchedTransaction.status}`);
                console.log(`[Fiat] 🔍 Expected status: ${TransactionStatus.PENDING}`);
                console.log(`[Fiat] 🔍 Status comparison: ${matchedTransaction.status === TransactionStatus.PENDING}`);
                // Check if transaction is already processed or being processed
                if (matchedTransaction.status !== TransactionStatus.PENDING) {
                    console.log(`[Fiat] ❌ Transaction ${matchedTransaction.transactionId} already processed with status: ${matchedTransaction.status}`);
                    return;
                }
                console.log(`[Fiat] ✅ Transaction status is PENDING, proceeding with update...`);
                // Update the matched transaction status with atomic operation
                try {
                    await this.transactionService.updateFiatStatusTransactionHash(matchedTransaction.id, TransactionStatus.FIAT_CONFIRMED, transactionReference);
                }
                catch (error) {
                ...
                if (!matchedTransaction.destinationAddress ||
                    ...
                    destinationTransfer = {
                        success: false,
                        error: "Missing required destination fields (address, chain, currency, or amount)",
                    };
                }
                else {
                    try {
                        ...
                        console.log(`[Fiat] 📊 Token transfer result:`, destinationTransfer);
                    }
                    catch (error) {
                ...
                if (!destinationTransfer.success) {
                    ...
                    await this.transactionService.updateTransactionFields(matchedTransaction.id, {
                ...
                await this.transactionService.updateTransactionFields(matchedTransaction.id, {
                    ...
                    adminNotes: "Transaction completed successfully - funds transferred to address",
                });
            }
            else {
                console.log(`⚠️ User not found for payment notification. 📧 Customer email: ${customer?.email}`);
                return;
            }
        }
        catch (error) {
    ...
    async handleFailedPayment(eventData) {
        try {
            const { customer, transactionReference, paymentReference, amountPaid, currency, paymentStatus, } = eventData;
    ...
    async handleReversedPayment(eventData) {
        try {
            const { customer, amountPaid, transactionReference, paymentReference, currency, settlementAmount, paidOn, paymentDescription, } = eventData;
