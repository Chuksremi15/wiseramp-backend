...
import { getTokenEquivalent } from "../utils/price-feed.js";
...
export class TransactionController extends BaseController {
     ...
     */
    async getSourceAddress(userId, sourceChain) {
    ...
    createFiatToCryptoTransaction = this.asyncHandler(async (req, res) => {
        if (!req.user) {
        ...
        const { sourceAmount, sourceCurrency, sourceChain, destinationCurrency, destinationAddress, destinationChain, } = req.body;
        // Validation
        ...
        if (sourceCurrency !== "NGN") {
            return this.sendError(res, "Source currency must be NGN for fiat-to-crypto transactions");
        }
        ...
        if (!user.reserveAccounts || user.reserveAccounts.length === 0) {
            try {
                ...
                if (!updatedUser?.reserveAccounts ||
                    updatedUser.reserveAccounts.length === 0) {
                    return this.sendError(res, "Failed to create reserve account. Please try again.");
                }
                ...
                return this.sendError(res, "Failed to create reserve account. Please try again later.");
            }
        }
        const { accountNumber, bankName, accountName } = user.reserveAccounts[0];
        // Validate sourceAmount is a valid number
        let numericSourceAmount;
        try {
            numericSourceAmount = this.validateAndParseAmount(sourceAmount);
        }
        catch (error) {
            return this.sendError(res, error instanceof Error
                ? error.message
                : "Invalid source amount. Must be a positive number.");
        }
        const { destinationEquivalent, exchangeRate } = await getTokenEquivalent(destinationCurrency, sourceCurrency, numericSourceAmount);
        const feeAmount = new Decimal(numericSourceAmount).mul(0.006).toNumber();
        const netAmount = new Decimal(numericSourceAmount)
            .sub(feeAmount)
            .toNumber();
        const transactionId = await this.transactionService.createFiatToCryptoTransaction({
            userId: id,
            ...
            destinationAddress: normalizeAddress(destinationAddress, destinationChain),
            exchangeRate: exchangeRate.toString(),
            feeAmount: feeAmount.toString(),
            netAmount: netAmount.toString(),
        });
        const transaction = await this.transactionService.getTransactionById(transactionId);
        return this.sendSuccess(res, { transaction }, "Fiat-to-crypto transaction created successfully", 201);
    });
    // Create a crypto-to-fiat transaction
    createCryptoToFiatTransaction = this.asyncHandler(async (req, res) => {
        if (!req.user) {
            return this.sendError(res, "User not authenticated", 401);
        }
        const { id: userId } = req.user;
        const { sourceAmount, sourceCurrency, sourceChain, destinationCurrency, destinationChain, bankAccountId, } = req.body;
        // Validation
        const validationError = this.validateRequiredFields(req.body, [
            "sourceAmount",
            "sourceCurrency",
        ...
        const bankAccount = await BankAccountService.getBankAccountById(bankAccountId, userId);
        if (!bankAccount) {
        ...
        let sourceAddress;
        try {
            sourceAddress = await this.getSourceAddress(userId.toString(), sourceChain);
        }
        catch (error) {
            return this.sendError(res, error instanceof Error
                ? error.message
        ...
        const netAmount = new Decimal(numericSourceAmount)
            .sub(feeAmount)
            .toNumber();
        const transactionId = await this.transactionService.createCryptoToFiatTransaction({
            userId: userId,
            sourceChain: sourceChain,
            sourceAmount: numericSourceAmount.toString(),
            sourceCurrency: sourceCurrency,
        ...
        const transaction = await this.transactionService.getTransactionById(transactionId);
        return this.sendSuccess(res, { transaction }, "Crypto-to-fiat transaction created successfully", 201);
    });
    // Create a crypto-to-crypto transaction
    createCryptoToCryptoTransaction = this.asyncHandler(async (req, res) => {
        if (!req.user) {
            return this.sendError(res, "User not authenticated", 401);
        }
        const { id: userId } = req.user;
        const { sourceAmount, sourceCurrency, destinationCurrency, destinationAddress, sourceChain, destinationChain, } = req.body;
        // Validation
        const validationError = this.validateRequiredFields(req.body, [
            "sourceAmount",
            "sourceCurrency",
        ...
        let exchangeRate;
        try {
            const result = await getTokenEquivalent(destinationCurrency.toUpperCase(), sourceCurrency.toUpperCase(), numericSourceAmount, destinationChain);
        ...
        const transactionId = await this.transactionService.createCryptoToCryptoTransaction({
            userId: userId,
            sourceChain: sourceChain,
            destinationChain: destinationChain,
            sourceAmount: numericSourceAmount.toString(),
        ...
        const transaction = await this.transactionService.getTransactionById(transactionId);
        return this.sendSuccess(res, { transaction }, "Crypto-to-crypto transaction created successfully", 201);
    });
    ...
    getUserTransactions = this.asyncHandler(async (req, res) => {
        if (!req.user) {
            return this.sendError(res, "User not authenticated", 401);
        }
        const { id: userId } = req.user;
        const { page = 1, limit = 10, status } = req.query;
        const transactions = await this.transactionService.getTransactionsByUserId(userId, Number(limit));
    ...
    updateCryptoStatus = this.asyncHandler(async (req, res) => {
        const { id } = req.params;
        ...
        const success = await this.transactionService.updateCryptoStatus(id, status, txHash);
        if (!success) {
            return this.sendError(res, "Transaction not found", 404);
        }
...
export const { createFiatToCryptoTransaction, createCryptoToFiatTransaction, createCryptoToCryptoTransaction, getTransactionById, getUserTransactions, updateCryptoStatus, } = transactionController;
