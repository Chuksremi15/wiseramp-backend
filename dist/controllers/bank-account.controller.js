...
export class BankAccountController extends BaseController {
     ...
     */
    createBankAccount = this.asyncHandler(async (req, res) => {
        if (!req.user) {
        ...
        try {
            ...
            return this.sendSuccess(res, { data: bankAccount }, "Bank account created successfully", 201);
        }
        catch (error) {
     ...
     */
    getUserBankAccounts = this.asyncHandler(async (req, res) => {
        if (!req.user) {
            return this.sendError(res, "User not authenticated", 401);
        }
        const { id: userId } = req.user;
     ...
     */
    getBankAccountById = this.asyncHandler(async (req, res) => {
        if (!req.user) {
            return this.sendError(res, "User not authenticated", 401);
        }
        const { id: userId } = req.user;
        ...
        const bankAccount = await BankAccountService.getBankAccountById(accountId, userId);
        if (!bankAccount) {
     ...
     */
    updateBankAccount = this.asyncHandler(async (req, res) => {
        if (!req.user) {
            return this.sendError(res, "User not authenticated", 401);
        }
        const { id: userId } = req.user;
        ...
        if (bankCode !== undefined)
            updates.bankCode = bankCode.trim();
        // Check if there are any updates
        if (Object.keys(updates).length === 0) {
            return this.sendError(res, "No valid fields to update");
        }
        try {
            const updatedAccount = await BankAccountService.updateBankAccount(accountId, userId, updates);
            if (!updatedAccount) {
                return this.sendError(res, "Bank account not found or access denied", 404);
            }
            return this.sendSuccess(res, { data: updatedAccount }, "Bank account updated successfully");
        }
        catch (error) {
            if (error instanceof Error) {
                if (error.message.includes("already exists")) {
     ...
     */
    deleteBankAccount = this.asyncHandler(async (req, res) => {
        if (!req.user) {
            return this.sendError(res, "User not authenticated", 401);
        }
        const { id: userId } = req.user;
        ...
        const deleted = await BankAccountService.deleteBankAccount(accountId, userId);
        if (!deleted) {
            return this.sendError(res, "Bank account not found or access denied", 404);
        }
     ...
     */
    getBankAccountByNumber = this.asyncHandler(async (req, res) => {
        if (!req.user) {
            return this.sendError(res, "User not authenticated", 401);
        }
        const { id: userId } = req.user;
        ...
        const bankAccount = await BankAccountService.getBankAccountByNumber(accountNumber, userId);
        if (!bankAccount) {
            return this.sendError(res, "Bank account not found", 404);
        }
        return this.sendSuccess(res, { data: bankAccount });
     ...
     */
    checkUserHasBankAccounts = this.asyncHandler(async (req, res) => {
        if (!req.user) {
            return this.sendError(res, "User not authenticated", 401);
        }
        const { id: userId } = req.user;
        const hasBankAccounts = await BankAccountService.userHasBankAccounts(userId);
        const accountCount = await BankAccountService.getUserBankAccountCount(userId);
        return this.sendSuccess(res, {
            data: {
                hasBankAccounts,
                accountCount,
...
export const { createBankAccount, getUserBankAccounts, getBankAccountById, updateBankAccount, deleteBankAccount, getBankAccountByNumber, checkUserHasBankAccounts, } = bankAccountController;
