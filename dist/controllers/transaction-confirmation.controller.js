...
export class TransactionConfirmationController extends BaseController {
    ...
    onConfirmedEvmTx = this.asyncHandler(async (req, res) => {
        ...
        const result = await this.confirmationService.processConfirmedEvmTransaction(transactionId);
        if (!result.success) {
            return this.sendError(res, result.error, 500);
        }
        return this.sendSuccess(res, result.data, "Transaction confirmed and processed successfully");
    });
}
// Export controller instance methods for backward compatibility
const transactionConfirmationController = new TransactionConfirmationController();
