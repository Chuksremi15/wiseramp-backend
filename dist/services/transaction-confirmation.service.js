...
export class TransactionConfirmationService {
     ...
     */
    async validateTransactionForConfirmation(transactionId) {
     ...
     */
    async handleBalanceVerificationFailure(transaction, currency, error) {
        console.log(`[CONFIRMATION] ${currency} balance verification failed for ${transaction.sourceAddress}: ${error}`);
        // Update transaction status to indicate balance verification failed
        const response = await this.transactionService.updateTransactionFields(transaction.id, {
     ...
     */
    async handleTransferFailure(transaction, transferType, cryptoStatus, error) {
     ...
     */
    async processConfirmedEvmTransaction(transactionId) {
        try {
            // Validate required fields
            if (!transactionId) {
                return { success: false, error: "Transaction ID is required" };
            }
            console.log(`[CONFIRMATION] Processing confirmed transaction: ${transactionId}`);
            // 1. Verify the transaction exists and is in correct status
            const { success: validatationSuccess, error: validationError, transaction, } = await this.validateTransactionForConfirmation(transactionId);
            if (!validatationSuccess || !transaction) {
                return { success: false, error: validationError };
            }
            const tokenInfo = TokenConfigUtils.getTokenInfo(transaction.sourceChain, transaction.sourceCurrency);
            //2. Check if the address actually has the expected balance
            //Note: WalletTransferService needs to be imported
            const balanceCheck = await WalletTransferService.verifyTokenBalance(transaction.sourceAddress, Number(transaction.sourceAmount), transaction.sourceCurrency, tokenInfo?.address);
            if (!balanceCheck.success) {
                await this.handleBalanceVerificationFailure(transaction, transaction.sourceCurrency, balanceCheck.error);
                return { success: false, error: balanceCheck.error };
            }
            console.log(`[CONFIRMATION] Balance verified: ${transaction.sourceAddress} has ${balanceCheck.balance} ${transaction.sourceCurrency}`);
            ...
            if (!transaction.sourceAddress ||
                ...
                return {
                    success: false,
                    error: "Missing required fields: sourceAddress, sourceChain, or sourceCurrency",
                };
            }
            ...
            if (!queueResult.success) {
                console.error(`[CONFIRMATION] ${transaction.sourceCurrency.toUpperCase()} Failed to queue transfer: ${queueResult.error}`);
            }
            else {
                console.log(`[CONFIRMATION] ${transaction.sourceCurrency.toUpperCase()} transfer queued for background processing, proceeding to destination transfer`);
            }
            ...
            switch (transaction.destinationChain) {
                case Chain.FIAT: {
                    if (!transaction.destinationBankAccountNumber ||
                        !transaction.destinationBankCode) {
                        destinationTransfer = {
                            success: false,
                            error: "Destination account name or bank code is missing for fiat transfer",
                        };
                        break;
                    }
            ...
            if (!destinationTransfer.success) {
                return await this.handleTransferFailure(transaction, `${transaction.destinationChain} transfer`, TransactionStatus.TOKEN_FROM_VAULT_TRANSFER_FAILED, destinationTransfer.error);
            }
            ...
            console.log(`[CONFIRMATION] Transaction ${transactionId} completed successfully`);
            return {
                success: true,
                data: {
                    transactionId,
