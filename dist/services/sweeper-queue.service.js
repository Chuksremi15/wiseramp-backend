...
import { transferQueue, } from "../db/schema.js";
...
export class SweeperQueueService {
     ...
     */
    static async queueTransfer({ transactionId, userId, fromAddress, amount, sourceChain, sourceCurrency, }) {
        try {
            ...
            const [insertedTransfer] = await db
                ...
                .values(newTransfer)
                .returning();
            console.log(`[QUEUE] Transfer queued for transaction ${transactionId} with ID ${insertedTransfer.id}`);
            // Process immediately in background (don't await)
            this.processQueuedTransferById(insertedTransfer.id).catch((error) => {
                console.error(`[QUEUE] Background processing failed for ${transactionId}:`, error);
            });
            return { success: true, queueId: insertedTransfer.id };
        }
        catch (error) {
            console.error(`[QUEUE] Failed to queue transfer for ${transactionId}:`, error);
     ...
     */
    static async processQueuedTransferById(queueId) {
        try {
            const db = getDatabase();
            ...
            console.log(`[QUEUE] Processing transfer for transaction ${queuedTransfer.transactionId}`);
            ...
            if (!queuedTransfer.sourceChain || !queuedTransfer.sourceCurrency) {
                throw new Error("Missing required fields: sourceChain or sourceCurrency");
            }
            ...
            if (transferResult.success) {
                ...
                await this.getTransactionService().updateTransactionFieldsByTransactionId(queuedTransfer.transactionId, {
                    ...
                    sweepAdminNotes: "Background transfer to vault completed successfully",
                });
                console.log(`[QUEUE] Transfer completed for transaction ${queuedTransfer.transactionId}`);
            }
            else {
            ...
            await this.handleTransferFailureById(queueId, `Processing error: ${error}`);
        }
    }
    /**
     * Handle transfer failure with retry logic
     */
    static async handleTransferFailureById(queueId, error) {
        try {
            const db = getDatabase();
            ...
            const [queuedTransfer] = await db
                ...
                .where(eq(transferQueue.id, queueId));
            if (!queuedTransfer) {
                console.error(`[QUEUE] Transfer with ID ${queueId} not found for failure handling`);
                return;
            }
            ...
            if (newRetryCount < queuedTransfer.maxRetries) {
                ...
                console.log(`[QUEUE] Scheduling retry ${newRetryCount}/${queuedTransfer.maxRetries} for transaction ${queuedTransfer.transactionId} in ${retryDelay}ms`);
                setTimeout(() => {
                    this.processQueuedTransferById(queueId).catch((retryError) => {
                        console.error(`[QUEUE] Retry failed for ${queuedTransfer.transactionId}:`, retryError);
                    });
                }, retryDelay);
            }
            else {
                ...
                console.error(`[QUEUE] Transfer permanently failed for transaction ${queuedTransfer.transactionId} after ${queuedTransfer.maxRetries} attempts`);
            }
        }
        catch (dbError) {
            console.error(`[QUEUE] Database error while handling transfer failure:`, dbError);
        }
    }
    /**
     * Get queue status for a transaction
     */
    static async getQueueStatus(transactionId) {
        try {
            const db = getDatabase();
            const [queuedTransfer] = await db
                .select()
            ...
            console.error(`[QUEUE] Error getting queue status for ${transactionId}:`, error);
            return null;
        }
    }
    /**
     ...
     */
    static async getFailedTransfers() {
        try {
            const db = getDatabase();
            return await db
                .select()
     ...
     */
    static async retryTransfer(queueId) {
        try {
            const db = getDatabase();
            const [queuedTransfer] = await db
                .select()
            ...
            this.processQueuedTransferById(queueId).catch((error) => {
                console.error(`[QUEUE] Manual retry failed for queue ID ${queueId}:`, error);
            });
            return { success: true };
        }
        catch (error) {
