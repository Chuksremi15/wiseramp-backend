...
export class UserAddressService {
     ...
     */
    static async getAddressesByChain(chain, userId) {
        try {
            const db = getDatabase();
     ...
     */
    static async updateUserAddress(addressId, userId, updates) {
        try {
            // Verify ownership
            ...
            const [updatedAddress] = await db
                ...
                .where(and(eq(UserAddress.id, addressId), eq(UserAddress.userId, userId)))
                .returning();
     ...
     */
    static async userHasAddresses(userId) {
        try {
            const db = getDatabase();
            const [address] = await db
     ...
     */
    static async getUserAddressCount(userId) {
        try {
            const db = getDatabase();
            const addresses = await db
                .select({ id: UserAddress.id })
