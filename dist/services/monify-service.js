...
export class MonifyService {
     ...
     */
    async createReserveAccount(data) {
        const { email } = data;
        if (!email) {
        ...
        try {
            const response = await monifyAxios.post("/api/v2/bank-transfer/reserved-accounts", accountRequestData);
            ...
            throw new Error(`Failed to create reserve account: ${error.response?.data?.message || error.message}`);
        }
    }
    ...
    async executeVaultTransfer(params) {
        try {
            ...
            try {
                ...
                const response = await monifyAxios.post("/api/v2/disbursements/single", transferData);
    ...
    async getReserveAccount(data) {
        ...
        try {
            const response = await monifyAxios.get(`/api/v2/bank-transfer/reserved-accounts/${user.reserveAccountRef}`);
            return response.data;
        }
        catch (error) {
            console.error("Get reserve account error:", error);
            throw new Error(`Failed to get reserve account: ${error.response?.data?.message || error.message}`);
        }
    }
}
