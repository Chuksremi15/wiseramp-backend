...
export class BankAccountService {
    ...
    static async createBankAccount(data) {
        try {
            ...
            const existingAccount = await db
                .select()
                .from(BankAccount)
                .where(and(eq(BankAccount.userId, data.userId), eq(BankAccount.accountNumber, data.accountNumber)))
                .limit(1);
     ...
     */
    static async getBankAccountById(accountId, userId) {
        try {
     ...
     */
    static async getBankAccountByNumber(accountNumber, userId) {
        try {
     ...
     */
    static async updateBankAccount(accountId, userId, updates) {
        try {
            // Verify ownership
            ...
            if (updates.accountNumber &&
                ...
                const duplicate = await this.getBankAccountByNumber(updates.accountNumber, userId);
     ...
     */
    static async deleteBankAccount(accountId, userId) {
        try {
            const db = getDatabase();
     ...
     */
    static async userHasBankAccounts(userId) {
        try {
            const db = getDatabase();
            const [account] = await db
     ...
     */
    static async getUserBankAccountCount(userId) {
        try {
            const db = getDatabase();
            const accounts = await db
                .select({ id: BankAccount.id })
