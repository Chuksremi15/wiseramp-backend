...
import { pgTable, text, integer, decimal, timestamp, pgEnum, index, uniqueIndex, serial, jsonb, boolean, } from "drizzle-orm/pg-core";
import { sql } from "drizzle-orm";
export const chainEnum = pgEnum("chain", Object.values(Chain));
export const transactionStatusEnum = pgEnum("transaction_status", Object.values(TransactionStatus));
...
export const users = pgTable("users", {
    ...
    reserveAccounts: jsonb("reserve_accounts").$type(),
...
export const transactions = pgTable("transactions", {
    ...
    sourceChainStatusIdx: index("source_chain_status_idx").on(table.sourceChain, table.cryptoStatus, table.sourceAddress),
    expiredStatusIdx: index("expired_status_idx").on(table.expiredAt, table.status),
    userCreatedIdx: index("user_created_idx").on(table.userId, table.createdAt),
    sourceAddressStatusIdx: index("source_address_status_idx").on(table.sourceAddress, table.cryptoStatus),
    statusExpiredIdx: index("status_expired_idx").on(table.status, table.expiredAt),
}));
...
export const transferQueue = pgTable("transfer_queue", {
    ...
    index("transfer_queue_status_created_idx").on(table.status, table.createdAt),
