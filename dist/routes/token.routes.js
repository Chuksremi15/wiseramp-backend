...
import { TokenConfigUtils, COMMON_TOKENS, } from "../utils/token-config.js";
...
router.get("/chains/:chain/tokens", (req, res) => {
    try {
...
router.get("/chains/:chain/tokens/:symbol", (req, res) => {
    try {
...
router.get("/resolve/:chain/:symbol", (req, res) => {
    try {
        const { chain, symbol } = req.params;
...
router.get("/check/:chain/:symbol", (req, res) => {
    try {
        const { chain, symbol } = req.params;
...
router.post("/monitor", async (req, res) => {
    try {
        ...
        if (tokenSymbol) {
            ...
            success = await hypersyncWorker.addAddressForToken(address, chain, tokenSymbol);
            if (!success) {
                return res.status(400).json({
        ...
        res.json({
            success: true,
            message: `Started monitoring ${address} on ${chain}${tokenSymbol ? ` for ${tokenSymbol}` : " for ETH/native token"}`,
...
router.get("/hypersync/status", (req, res) => {
    try {
...
router.post("/format", (req, res) => {
    try {
        ...
        const formattedAmount = TokenConfigUtils.formatTokenAmount(amount, chain, tokenSymbol);
