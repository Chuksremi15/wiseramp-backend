...
async function main() {
  ...
  let totalERC20Volume: Record<string, Record<string, bigint>> = {};
  let totalWeiVolume: Record<string, bigint> = {};
  ...
  while (true) {
    ...
    if (res.data.logs.length > 0) {
      for (const log of res.data.logs) {
        ...
        const amount = event.body[0].val as bigint;
        ...
        if (addresses.includes(from) || addresses.includes(to)) {
          console.log(
            `\x1b[90mERC20 Transfer: ${tokenAddress} from ${from} to ${to}, amount: ${amount}\x1b[0m`
          );
        }
      }
    }
    ...
    for (const tx of res.data.transactions) {
      ...
      if (!tx.from || tx.value === undefined) continue;
      ...
      if (addresses.includes(from) || addresses.includes(to)) {
        console.log(
          `\x1b[31mETH TX: ${tx.hash} from ${from} to ${to}, value: ${value} wei\x1b[0m`
        );
      }
    }
