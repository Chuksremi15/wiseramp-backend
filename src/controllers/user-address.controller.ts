...
export class UserAddressController extends BaseController {
   ...
   */
  createUserAddress = this.asyncHandler(
   ...
   */
  getUserAddresses = this.asyncHandler(
    async (req: Request, res: Response): Promise<Response | void> => {
      if (!req.user) {
        return this.sendError(res, "User not authenticated", 401);
      }
   ...
   */
  getAddressById = this.asyncHandler(
    async (req: Request, res: Response): Promise<Response | void> => {
      if (!req.user) {
        return this.sendError(res, "User not authenticated", 401);
      }
   ...
   */
  getAddressesByChain = this.asyncHandler(
    async (req: Request, res: Response): Promise<Response | void> => {
      if (!req.user) {
        return this.sendError(res, "User not authenticated", 401);
      }
      ...
      const addresses = await UserAddressService.getAddressesByChain(
        chain,
        userId
      );
   ...
   */
  updateUserAddress = this.asyncHandler(
    async (req: Request, res: Response): Promise<Response | void> => {
      if (!req.user) {
        return this.sendError(res, "User not authenticated", 401);
      }
      ...
      const updates: any = {};
      if (chain !== undefined) updates.chain = chain.trim();
      if (addressName !== undefined) updates.addressName = addressName.trim();
      if (userAddress !== undefined) updates.userAddress = userAddress.trim();
   ...
   */
  deleteUserAddress = this.asyncHandler(
    async (req: Request, res: Response): Promise<Response | void> => {
      if (!req.user) {
        return this.sendError(res, "User not authenticated", 401);
      }
      ...
      const deleted = await UserAddressService.deleteUserAddress(
        addressId,
        userId
      );
   ...
   */
  checkUserHasAddresses = this.asyncHandler(
    async (req: Request, res: Response): Promise<Response | void> => {
      if (!req.user) {
        return this.sendError(res, "User not authenticated", 401);
      }
